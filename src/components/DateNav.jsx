...
export default function DateNav({ selectedDate, onChange }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-200 shadow-sm p-3 flex items-center gap-2">
      <button onClick={() => onChange(addDays(selectedDate, -1))} className="w-9 h-9 rounded-lg hover:bg-stone-100 flex items-center justify-center text-stone-500">
        <ChevronLeft size={18} />
      </button>
      ...
      {!isToday && (
        <button onClick={() => onChange(todayStr())} className="ml-1 text-xs font-medium text-emerald-700 bg-emerald-50 hover:bg-emerald-100 px-3 py-1.5 rounded-full shrink-0">
          Hôm nay
        </button>
      )}
    </div>
